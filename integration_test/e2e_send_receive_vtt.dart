...
Future<void> e2eSendReceiveTest(WidgetTester tester) async {
  ...
  await tester.pumpAndSettle();
  await tapButton(tester, localization.sendReceiveTx, semantics: true);
  await tapButton(tester, localization.send);
  expect(widgetByText(localization.transaction), findsOneWidget);
  await tapButton(tester, localization.sendReceiveTx, semantics: true);
  await tapButton(tester, localization.receive);
  expect(widgetByText(localization.transaction), findsNothing);
  expect(widgetByText(localization.copyAddressLabel), findsOneWidget);
